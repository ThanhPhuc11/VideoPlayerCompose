...
class MetaDataReaderImpl(
    ...
    override fun getMetaDataFromUri(contentUri: Uri): MetaData? {
        ...
        val fileName = app.contentResolver
            .query(
                contentUri,
                arrayOf(MediaStore.Video.VideoColumns.DISPLAY_NAME, MediaStore.Video.Media.DATA, MediaStore.Video.VideoColumns.DATA),
                null,
                null,
                null,
            )
...
//                fileName = Uri.parse(fullFileName).path ?: return null
            )
        }
    }
}